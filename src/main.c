...
int	ft_init_time(t_time *time, size_t *philo_has_eaten, size_t *ready)
{
	time->ready = ready;
...
int	ft_init_sim(t_simulation *sim, char **av, pthread_t *time_thread)
{
	...
	if (ft_init_time(&sim->time, &sim->philo_has_eaten, &sim->ready))
		return (1);
	if (pthread_create(time_thread, NULL, ft_time_routine, &sim->time))
		return (1);
...
int	main(int ac, char *av[])
{
	...
	if (ft_check_args(av + 1))
		return (1);
	if (ft_init_sim(&sim, av, &time_thread))
		return (1);
